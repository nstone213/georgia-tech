...
class Map:
    ...
    def is_inbound(self, node):
        ...
        """
        half_of_width = self.width / 2
        half_of_height = self.height / 2
        return (-half_of_width <= node.x <= half_of_width and -half_of_height <= node.y <= half_of_height)
    ...
    def is_inside_obstacles(self, node, use_all_obstacles = False):
        ...
        use_all_obstacles: 
        if True, check if the node is inside all obstacles

        Hint 1: Treat the obstacles as rectangles, padding is already done for you.
        Hint 2: Consider what other obstacle subsets are useful for this function.
        ...
        """
        obstacles = self._obstacles if use_all_obstacles else (self._explored_obstacles if self._exploration_mode else self._obstacles)
        
        for obstacle in obstacles:
            x_direction = [n.x for n in obstacle]
            y_direction = [n.y for n in obstacle]
            
            x_min = min(x_direction)
            x_max = max(x_direction)
            y_min = min(y_direction)
            y_max = max(y_direction)
    ...
    def step_from_to(self, node0, node1, limit=75):
        ...
        if dist <= limit:
            return Node((node1.x, node1.y))
        
        rat = limit / dist
        x_new = node0.x + d_x * rat
        y_new = node0.y + d_y * rat
    ...
    def node_generator(self):
        ...
        while rand_node is None:
            if np.random.random() < 0.1 and self._goals:
                goaled = self._goals[0]
                rand_node = Node((goaled.x, goaled.y))
            else:
            ...
            if not self.is_inbound(rand_node) or self.is_inside_obstacles(rand_node):
                rand_node = None
    ...
    def compute_smooth_path(self, path, iterations):
        ...
        if not self.is_collision_with_obstacles((path[0], path[-1])):
            return [path[0], path[-1]]
        
        middle = len(path) // 2
        first = self.compute_smooth_path(path[:middle+1], iterations-1)
        second = self.compute_smooth_path(path[middle:], iterations-1)
