...
def RRT(map):
    ...
    while (map.get_num_nodes() < MAX_NODES):
        random_node = map.get_random_valid_node()
        node_near = None
        ...
        newest_node = map.step_from_to(node_near, random_node)
        
        if not map.is_collision_with_obstacles((node_near, newest_node)):
            map.add_path(node_near, newest_node)
            
            for goal in map.get_goals():
                if get_dist(newest_node, goal) < 15 and not map.is_collision_with_obstacles((newest_node, goal)):
                    map.add_path(newest_node, goal)
                    break
