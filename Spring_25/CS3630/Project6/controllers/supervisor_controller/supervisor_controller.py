...
if __name__ == "__main__":
    ...
    # Enable the emitter for robot position broadcasting
    emitter = supervisor.getDevice("emitter")
    emitter.setChannel(0)
    # emitter.enable(timestep)
    
    # Get robot nodes by DEF name
    epuck = supervisor.getFromDef("e-puck")
    disrupter1 = supervisor.getFromDef("Disrupter1")
    disrupter2 = supervisor.getFromDef("Disrupter2")

    robots = [epuck, disrupter1, disrupter2]

    while supervisor.step(timestep) != -1:
        robot_poses = {}
        for robot in robots:
            name = robot.getField("name").getSFString()
            position = robot.getPosition()
            orientation = robot.getOrientation()

            heading = -math.atan2(orientation[1], orientation[0])

            # Store pose in dictionary
            robot_poses[name] = {
                "x": round(position[0], 2),
                "y": round(position[1], 2),  # Webots uses Y-up, so we use Z for 2D Y
                "h": round(heading, 2)
            }

        # Convert to JSON and send
        json_message = json.dumps(robot_poses)
        # print(json_message)
